		...
		function add_menu_pages() {
			...
			if(is_multisite() && (defined('PO_GLOBAL') && PO_GLOBAL == true)) {
				if(function_exists('is_network_admin') && is_network_admin()) {
					add_menu_page(__('Pop Overs','popover'), __('Pop Overs','popover'), $perms,  'popover', array(&$this,'handle_popover_admin'), PO_IMG_URL . 'window.png' );
				}
			} else {
				if(!function_exists('is_network_admin') || !is_network_admin()) {
					add_menu_page(__('Pop Overs','popover'), __('Pop Overs','popover'), $perms,  'popover', array(&$this,'handle_popover_admin'), PO_IMG_URL . 'window.png' );
					...
					<tbody>
						...
						if($addons) {
							foreach($addons as $key => $addon) {
								...
								$addon_data = get_file_data( PO_INC_DIR . 'addons/' . $addon, $default_headers, 'plugin' );
