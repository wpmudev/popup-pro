...
class popoverajax {
		...
		if(!empty($popovers)) {
			...
			foreach( (array) $popovers as $popover ) {
				...
				if(!empty($popover_check)) {
					...
					if(isset($_REQUEST['po_id']) && (int) $_REQUEST['po_id'] != 0) {
						if($popover->id == (int) $_REQUEST['po_id']) {
							$show = true;
						} else {
							$show = false;
						}
				...
				if($show == true) {
					...
					return apply_filters('popover-output-popover', $popover, $_original_popover);
	...
	function clear_forever($id=false) {
	...
	static public function get_country_from_api( $ip ) {
